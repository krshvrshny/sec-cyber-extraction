...
# ── LOAD & MERGE DATA ─────────────────────────────────────────────────────────
df = pd.read_excel("content_scores.xlsx")
length = pd.read_csv("length_results.csv")[["ticker","year","sector","size","has_1c"]].drop_duplicates()
...
# ═════════════════════════════════════════════════════════════════════════════
yearly = df.groupby("year")["specificity_score"].agg(["mean","std"]).reset_index()

fig, ax = plt.subplots(figsize=(9, 5))

# Jittered individual firm dots
for year in sorted(df["year"].unique()):
    vals = df[df["year"] == year]["specificity_score"].values
...
ax.set_ylabel("Specificity Score (0–1)")
ax.set_ylim(-0.05, 1.1)
ax.legend(fontsize=10)
ax.set_title("Figure 9 – Mean Content Specificity Score Over Time\n"
             "(n=41 firms per year; dots = individual firm scores)",
             fontweight="bold")
plt.tight_layout()
plt.savefig("fig9_score_by_year.png", dpi=150, bbox_inches="tight")
plt.close()
print("Saved fig9_score_by_year.png")
...
ax.set_ylim(0, 115)
ax.legend(fontsize=10)
ax.set_title("Figure 11 – Category Adoption Rate: Filings With vs Without Item 1C",
             fontweight="bold")
plt.tight_layout()
plt.savefig("fig11_categories_by_1c.png", dpi=150, bbox_inches="tight")
plt.close()
print("Saved fig11_categories_by_1c.png")


# ═════════════════════════════════════════════════════════════════════════════
# FIG 12 – Mean specificity score by sector (bar + error bars)    [MAIN TEXT]
# ═════════════════════════════════════════════════════════════════════════════
sec = df.groupby("sector")["specificity_score"].agg(["mean","std","count"]).reindex(SECTOR_ORDER)
...
ax.set_xlabel("Mean Specificity Score (0–1)")
ax.set_xlim(0, 1.0)
ax.axvline(df["specificity_score"].mean(), color=ORANGE, ls="--", lw=1.5,
           label=f'Overall mean = {df["specificity_score"].mean():.3f}')
ax.legend(fontsize=10)
ax.set_title("Figure 12 – Mean Content Specificity Score by Sector\n"
             "(error bars = ±1 SD)",
             fontweight="bold")
plt.tight_layout()
plt.savefig("fig12_score_by_sector.png", dpi=150, bbox_inches="tight")
plt.close()
print("Saved fig12_score_by_sector.png")


# ═════════════════════════════════════════════════════════════════════════════
# FIG A4 – Specificity score boxplot by firm size                 [APPENDIX]
# ═════════════════════════════════════════════════════════════════════════════
size_means = df.groupby("size")["specificity_score"].mean()

fig, ax = plt.subplots(figsize=(8, 5))
sns.boxplot(data=df, x="size", y="specificity_score", order=SIZE_ORDER,
            color=BLUE, ax=ax,
            flierprops=dict(marker=".", markersize=5, alpha=0.5))

ymax = df["specificity_score"].quantile(0.98)
...
ax.set_title("Figure A4 – Content Specificity Score by Firm Size [Appendix]",
             fontweight="bold")
plt.tight_layout()
plt.savefig("figA4_score_by_size.png", dpi=150, bbox_inches="tight")
plt.close()
print("Saved figA4_score_by_size.png")


# ═════════════════════════════════════════════════════════════════════════════
# FIG A5 – Heatmap: all firms × all years                         [APPENDIX]
# ═════════════════════════════════════════════════════════════════════════════
pivot = df.pivot_table(index="ticker", columns="year", values="specificity_score")
pivot = pivot.sort_values(by=[2022, 2023, 2024, 2025])

fig, ax = plt.subplots(figsize=(7, 13))
sns.heatmap(pivot, ax=ax, annot=True, fmt=".2f", cmap="Blues",
            linewidths=0.4, linecolor="#e0e0e0",
            cbar_kws={"label": "Specificity Score", "shrink": 0.6},
            vmin=0, vmax=1,
            annot_kws={"size": 8})

ax.set_xlabel("Fiscal Year")
ax.set_ylabel("")
ax.set_title("Figure A5 – Content Specificity Score by Firm and Year [Appendix]\n"
             "(sorted by 2022 score ascending)",
             fontweight="bold")
plt.tight_layout()
plt.savefig("figA5_score_heatmap.png", dpi=150, bbox_inches="tight")
plt.close()
print("Saved figA5_score_heatmap.png")
